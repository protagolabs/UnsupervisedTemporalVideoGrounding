...
def save_checkpoint(state, day, second, t_abs,t_trip,flag):
    name = "{}/{}_tabs{}_ttrip{}_flag{}model.pth.tar".format(day, second,t_abs,t_trip,flag)
...
def main():
    ...
    t_trip = 0.5
    t_abs = 0.5
    saveflag = [0.01 for _ in range(6)]
    for bigepoch in range(30):
        savelist = []
        for d in range(necktype):
            ...
            optimizer = torch.optim.Adam(learned_params, lr=5e-4/(1+5*bigepoch))
            vidft_dic = {}
            for batchidx, (
                ...
                if bigepoch == 0:
                    for i, name in enumerate(names):
                        if not (name in vidft_dic.keys()):
                            agn_att = torch.tensor([1./lengths_vid[i] for _ in range(lengths_vid[i])]).to(device)
                            agn_att = agn_att.unsqueeze(0)
                            ft = target_vids[i,:lengths_vid[i], ]
                            vidft_dic[name] = torch.mm(agn_att, ft).squeeze(0).tolist()
                else:
                    spe_ft, spe_att, agn_atts, att, H = model(
                        torch.FloatTensor(neck_dict[d]).repeat(target_vids.size(0), 1, 1).to(device),
                        target_vids)
                    for i, name in enumerate(names):
                        if not (name in vidft_dic.keys()):
                            agn_att = agn_atts[i,:,:lengths_vid[i]]
                            ft = target_vids[i,:lengths_vid[i], ]
                            vidft_dic[name] = torch.mm(agn_att, ft).squeeze(0).tolist()
            vid_ft = torch.tensor(list(vidft_dic.values()))
        ...
        if iou_3_top_1 / total > saveflag[0]:
            saveflag[0] = iou_3_top_1 / total
            print('len:',len(savelist),'epoch:', bigepoch,saveflag[0],second, 'add to savelist')
            save_checkpoint({
                'state_dict_list': [model.state_dict() for model in savelist],
            }, day=day, second=31 ,t_abs = t_abs,t_trip=t_trip,flag=iou_3_top_1)
        if iou_5_top_1 / total > saveflag[1]:
            saveflag[1] = iou_5_top_1 / total
            print('len:',len(savelist),'epoch:', bigepoch,saveflag[1],second, 'add to savelist')
            save_checkpoint({
                'state_dict_list': [model.state_dict() for model in savelist],
            }, day=day, second=51 ,t_abs = t_abs,t_trip=t_trip,flag=iou_5_top_1)
        if iou_7_top_1 / total > saveflag[2]:
            saveflag[2] = iou_7_top_1 / total
            print('len:',len(savelist),'epoch:', bigepoch,saveflag[2],second, 'add to savelist')
            save_checkpoint({
                'state_dict_list': [model.state_dict() for model in savelist],
            }, day=day, second=71 ,t_abs = t_abs,t_trip=t_trip,flag=iou_7_top_1)
        if iou_3_top_5 / total > saveflag[3]:
            saveflag[3] = iou_3_top_5 / total
            print('len:',len(savelist),'epoch:', bigepoch,saveflag[3],second, 'add to savelist')
            save_checkpoint({
                'state_dict_list': [model.state_dict() for model in savelist],
            }, day=day, second=35 ,t_abs = t_abs,t_trip=t_trip,flag=iou_3_top_5)
        if iou_5_top_5 / total > saveflag[4]:
            saveflag[4] = iou_5_top_5 / total
            print('len:',len(savelist),'epoch:', bigepoch,saveflag[4],second, 'add to savelist')
            save_checkpoint({
                'state_dict_list': [model.state_dict() for model in savelist],
            }, day=day, second=55 ,t_abs = t_abs,t_trip=t_trip,flag=iou_5_top_5)
        if iou_7_top_5 / total > saveflag[5]:
            saveflag[5] = iou_7_top_5 / total
            print('len:',len(savelist),'epoch:', bigepoch,saveflag[5],second, 'add to savelist')
            save_checkpoint({
                'state_dict_list': [model.state_dict() for model in savelist],
            }, day=day, second=75 ,t_abs = t_abs,t_trip=t_trip,flag=iou_7_top_5)
